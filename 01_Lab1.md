...
2. Test the query [base]/Patient? in the browser. View the results using the HTML XML option (click on the appropriate links of the response).
...
6. Are there any patients living in Boston?
7. Extend the query with the parameter _format=html/xml
8. Test (and save!) the same query in the REST client. Note: With the REST client, no formatting information is required. The formatting is done by the client itself.
9. Configure your REST client with the header `Accept: application/fhir+xml` and save it as a favourite.
