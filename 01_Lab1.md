...
In our simple use case we want to work with **patients**, so this will be the main required resource. The documentation of the *Patient* resource is here: http://hl7.org/implement/standards/fhir/patient.html
...
Another aspect to consider ist which interactions are we going to need for the scenario. FHIR supports the following interactions organized in different interaction levels CRUD (create, read, update and delete) interactions. 
