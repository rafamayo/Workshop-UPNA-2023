...
Search results are always returned as a *Bundle* resource (https://www.hl7.org/fhir/bundle.html). The first part of the bundle resource contains metadata. The actual matches are contained within the repeatable element entry.
...
1. Identify appropriate attributes for the patient's last name and address
2. Apply the XML resource retrieved in the last task to the "Request Body" window. You need to perform the search and copy the content from the "Response Body (raw)" into the "Body (request body)" text box
3. Add Content-Type: application/fhir+xml to the header
4. Expand the resource to include the patient's first name and address. What is the required interaction? (Compare task 1)
